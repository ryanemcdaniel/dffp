...
resource "aws_lambda_permission" "api_discord" {
  ...
  source_arn    = "arn:aws:execute-api:${var.aws_region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.api_discord.id}/*/${aws_api_gateway_method.aws_api_gw_method.http_method}${aws_api_gateway_resource.api_discord.path}"
}
