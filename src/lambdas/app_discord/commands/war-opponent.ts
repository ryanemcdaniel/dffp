...
export const warOpponent = async (body: APIChatInputApplicationCommandInteraction) => {
    ...
    return pipe(
        ...
        reduceWithIndex('hit/def rate vs. hit/def rate\n', (idx, acc, [p1, p2]) => {
            ...
            const p1hrn = String(p1[1][1]).padEnd(3);
            const p1drn = String(p1[2][1]).padEnd(3);
            const p2Name = p2[0].name;
            const p2hr = (p2[1][0] * 100).toPrecision(3);
            const p2dr = (p2[2][0] * 100).toPrecision(3);
            const p2hrn = String(p2[1][1]).padEnd(3);
            const p2drn = String(p2[2][1]).padEnd(3);

            return acc
                + `**#${i} ${p1Name} vs. ${p2Name}**\n`
                + `-# ${p1hr}%, ${p1dr}%  (n=${p1hrn},${p1drn}) vs ${p2hr}%, ${p2dr}%  (n=${p2hrn},${p2drn})\n`;
        }),
    );
